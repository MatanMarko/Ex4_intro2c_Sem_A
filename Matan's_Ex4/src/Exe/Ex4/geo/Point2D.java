...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		double x = this.x()- cen.x();
		double y = this.y()- cen.y();
		double newX = cen.x() + (x*ratio);
		double newY = cen.y() + (y*ratio);
		this._x = newX;
		this._y = newY;
	}
	...
	public void rotate (Point2D cen, double angleDegrees) {
		...
		double radius=cen.distance(this);
		double angle = Math.atan(dy/dx);
		if(dx<0)
			angle +=Math.PI;
		angle+= Math.toRadians(angleDegrees);

		double newX=cen.x()+ radius*Math.cos(angle);
		double newY=cen.y()+ radius*Math.sin(angle);
